...
class UserManager:
    ...
    def autenticar(self):
        ...
        while intentos < MAX_LOGIN_INTENTOS:
            ...
            # Intento seguro de usar getpass(); si falla (sin TTY) usamos input() como fallback
            try:
    ...
    while True:
        ...
        if opc == '0':
            ...
            while True:
                ...
                print('v) Volver')
                choice = input('Opción usuarios: ').strip().lower()
                if choice == 'a':
                    usuario = pedir_str_no_vacio('Nuevo usuario: ')
                    contr = pedir_str_no_vacio('Contraseña: ')
                    rol = input('Rol (ADMIN/...): ').strip() or 'ADMIN'
                    resultado = um.crear(usuario, contr, rol)
                    print('Creado' if resultado else 'No creado (ya existe)')
                elif choice == 'b':
                    ...
                    usuario = pedir_str_no_vacio('Usuario a cambiar contraseña: ')
                    nueva = pedir_str_no_vacio('Nueva contraseña: ')
                    ok = um.cambiar_contrasena(usuario, nueva)
                    print('Contraseña cambiada' if ok else 'Usuario no existe')
                elif choice == 'd':
                    usuario = pedir_str_no_vacio('Usuario a eliminar: ')
                    ok = um.eliminar(usuario)
